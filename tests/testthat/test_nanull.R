...
test_that("works as expected", {
  ...
  expect_equal( nanull(data.frame() ), TRUE )
  ...
  expect_equal( nanull( data.frame(v1=character(),v2=numeric()) ) , FALSE ) # in the past this has incorrectly categorized an empty data frame as null.
  expect_equal( nanull( dplyr::group_by( dplyr::select( cars, speed, dist ), speed ) ), FALSE ) # test a tibble.
  
  expect_equal( 
    nanull( 
      list( 
        data.frame( some.data = c( 1, 2, 3, NA ) ),
        data.frame( some.data = c( 1, 2, 3, NA ) )
      )
    ), 
    FALSE 
